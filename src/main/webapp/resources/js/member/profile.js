var memberid=$("#memberid_hide").val();

$('#submit_modify').click(function () {
    if (!checkFullInput("#modify_form", ".alert-danger")) {
        $("#myModal").modal('hide');
        return false;
    }
    $('#myModal').modal('show');
...
$("#modify_sure").click(function () {
    $.ajax({
        type: 'get', url: '/member/j'+memberid+"/profile/modify",
        ...
        success: function (success) {
            if(success){
                window.location.href='/member/j'+memberid+'/profile';
            }else {
                $("#myModal").modal('hide');
                alert("修改失败");
            }
...
function uploadCanvas(canvas) {
    ...
    $.ajax({
        url: "/member/j"+memberid+"/profile/uploadImage",
        type: "POST",
        data: fd,
